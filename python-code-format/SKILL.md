...
核心原则：**任何人看到时不需要经过思考就能立刻明白它在干什么**，代码的最大成本不是写出来，而是后续反复阅读、理解和修改。能用朴素方案解决的，不要引入不必要的复杂度。注释的本质是替读者省去"推理代码意图"的时间，而不是复述语法。
...
简单的代码意味着维护成本更低——后来者（包括三个月后的你自己）不需要翻阅大量文件、解开层层抽象才能理解一处逻辑。复杂度应该是"被证明需要"之后才引入的，而非"万一将来需要"就提前铺设。
