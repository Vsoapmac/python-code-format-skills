...
这条适用于**所有注释文字**，包括模块头部 `@Description`、函数 docstring、类与变量的说明、行内注释、区块注释、region 注释、重要/警示注释、枚举成员注释。

注释是给后来的人看的，唯一的写法要求就是**说人话**：简单，直观。读的人扫一眼就懂，不用回头再想一遍。

- **不写听不懂的技术话**：不堆术语、不写只有作者懂的缩写
- **不写实现过程和痛点**：不解释"我是怎么绕过去的" "这里踩过什么坑"
- **不写无关信息**：与这段代码当前作用无关的话，一律不写

✅ `file_path = os.path.normpath(file_path) # 先规整路径, 避免同一个文件被当成两个`
❌ `file_path = os.path.normpath(file_path) # 消除路径中的 '.' 和 '..', 避免 isfile 误判`

两条写的是同一件事。前者一句话说清目的，后者要读完才明白在讲什么——区别就在说不说人话。
...
正文一律**说人话**（见开头「注释总则」）：简单，直观，不写听不懂的技术话，不写实现过程和痛点，不写无关信息。docstring 也不例外。
...
代码超过 8-10 行时使用成对 region 注释，横线统一 28 个 `-`。region 里的文字也要**说人话**，让人一眼知道这一块在干什么：

```python
# region ---------------------------- 读取并解析CSV文件 ----------------------------
...
# endregion ---------------------------- 读取并解析CSV文件 ----------------------------
```

## 重要/警示注释

对需谨慎修改、影响重大或不得已而为之的代码，用成对 `===` 分隔线包裹，单行文本不宜过长、注意换行。文字同样**说人话**，不要这里写一堆术语：
...
注释中统一使用**英文半角标点**（`()` `,` `:` `;` `.` `!` `?`），禁止中文全角标点；标点后必须加空格，英文括号内外侧与文字之间加空格。标点只影响怎么写，写什么内容仍以开头「注释总则」的说人话为准。
...
- 行内注释同样**说人话**（见开头「注释总则」），下面四类场景只是"该不该注释"，不是"可以写术语"的许可
...
**密度**：每 3-8 行代码至少一条，不要每行都加；判断标准是不熟悉该模块的人能否一眼看懂每块在干什么；注释分散在关键节点而非堆在一处。
...
生成任何代码必须**完整应用**上述格式；注释**说人话**、数量充分；避免无意义空行但保留分组空行；复杂代码用区域注释降低理解成本。综合效果示意（模块命名为 `CSVUtils.py`）：
...
def read_csv(file_path: str, delimiter: str = ",") -> List[Dict[str, str]]:
    ...
    # region ---------------------------- 参数校验与路径处理 ----------------------------
    file_path = os.path.normpath(file_path) # 先规整路径, 避免同一个文件被当成两个
    if not os.path.isfile(file_path): # 用 isfile 而不是 exists, 防止传进来一个目录
    ...
    rows_data = [] # 保存最终结果的列表
    try:
        # utf-8-sig 会跳过开头的 BOM, 避免列名多出 \ufeff 前缀
        with open(file_path, mode="r", encoding="utf-8-sig") as f:
            for row in csv.DictReader(f, delimiter=delimiter):
                rows_data.append(dict(row)) # 转成普通 dict, 避免 reader 的内容被改动
